...
@section('page-title', 'Author Overview')
...
@section('content')
    ...
    <div class="max-w-auto p-6 mb-4 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold">Manage Author</h1>
            <button data-modal-target="modalAddAuthor" data-modal-toggle="modalAddAuthor" id="modalAddAuthorButton"
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            $('#modalAddAuthorButton').on('click', function(e) {
                $('#name').val('');
                $('#email').val('');
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            DataTableUtils.refreshTable('author-table')
                                .then(() => {
                                    $('#modalAddAuthor').click();
                    ...
                    complete: function() {
                        $('#save-author').html(`<svg class="w-4 h-4 inline-block mr-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            let deleteId;
            $(document).on('click', 'a[href*="/author/"][href$="/delete"]', function(e) {
                e.preventDefault();
            ...
            $(document).on('click', '#edit-author', function(e) {
                ...
                let editEmail = $(this).attr('data-email');
                clearError('nameEdit');
                clearError('emailEdit');

                $('#modalEditAuthorButton').click();
                $.ajax({
                    url: `/author/${editId}/edit`,
                    type: 'GET',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
            ...
            $('#update-author').on('click', function(e) {
               ...
               // Clear previous errors
                ...
                $.ajax({
                    ...
                    complete: function() {
                        $('#update-author').html(`<svg class="w-4 h-4 inline-block mr-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12v6a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h6m5 4v3m0 0v3m0-3h3m-3 0h-3"/>
                    </svg>
                    Update Author`).prop('disabled', false);
                    },
                });
